void generateMainMethod() {
    ...
    Builder.CreateStore(llvm::ConstantFP::get(TheContext, llvm::APFloat(10.0f)), argAValAddr);

    llvm::Value* argB = Builder.CreateAlloca(realStructType, nullptr, "b");
    llvm::Value* argBValAddr = Builder.CreateStructGEP(realStructType, argB, 0);
    Builder.CreateStore(llvm::ConstantFP::get(TheContext, llvm::APFloat(20.0f)), argBValAddr);

    llvm::Function* returnRealFunction = TheModule->getFunction("returnReal");
    if (!returnRealFunction) {
        llvm::errs() << "returnReal function not found\n";
